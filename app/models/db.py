...
#     id = UnicodeAttribute(hash_key=True)  # Partition key
#     data = MapAttribute(null=True)  # Data field to store dynamic attributes
...
#         # Save the item to DynamoDB
#         record = SMSTable(id=item_id, name=item.get("name"), email=item.get("email"), phone_number=item.get("phone_number"), messages=updated_messages)
#         record.save()
#         return {"status": "Item added", "item": item}
#     except PynamoDBException as e:
#         raise Exception(str(e))
...
#         if "phone_number" in new_data:
#             record.phone_number = new_data["phone_number"]
...
#         # Save the updated item to DynamoDB
#         record.save()
#         return {"status": "Item updated", "item": {"id": record.id, "name": record.name, "email": record.email, "phone_number": record.phone_number, "messages": record.messages}}
#     except SMSTable.DoesNotExist:
#         raise ValueError("Item not found")
#     except PynamoDBException as e:
#         raise Exception(str(e))
...
# # update the way of creating item and updating item
# def get_or_create_item(item_id: str):
#     try:
#         # Retrieve item by `id`
#         record = SMSTable.get(hash_key=item_id)
#         return {"id": record.id, "name": record.name, "email": record.email, "phone_number": record.phone_number, "messages": record.messages}
#     except SMSTable.DoesNotExist:
#         # Create a new item if it does not exist
#         new_item = SMSTable(id=item_id, name="John Doe", email="", phone_number="", messages=[])
#         new_item.save()
#         return {"id": new_item.id, "name": new_item.name, "email": new_item.email, "phone_number": new_item.phone_number, "messages": new_item.messages}
#     except PynamoDBException as e:
#         raise Exception(str(e))
...
# # @app.get("/")
# # def read_root():
# #     return {"message": "Welcome to the FastAPI DynamoDB app"}
...
# #         item_id = item.get("id")
# #         if not item_id:
# #             raise HTTPException(status_code=400, detail="Missing 'id' in item")

# #         # Save the item to DynamoDB
# #         record = SMSTable(id=item_id, data=item)
# #         record.save()
# #         return {"status": "Item added", "item": item}
# #     except PynamoDBException as e:
